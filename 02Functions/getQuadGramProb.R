...
getQuadGramProb = function(inputString){
        # Example
        # inputString <- "bathroom and I asked about dessert and he started telling me about his horticultural"
        ...
        finalProb = -1
        oneGroupIn4Gram = QuadGramTable_ex[QuadGramTable_ex$FirstTerm == lastWords[length-3]
                                           & QuadGramTable_ex$Midterm1 == lastWords[length-2]
                                           & QuadGramTable_ex$Midterm2 == lastWords[length-1]]
        if (nrow(oneGroupIn4Gram) > 0){
                # Algorithm here:
                oneRecordIn4Gram = QuadGramTable_ex[QuadGramTable_ex$FirstTerm == lastWords[length-3]
                                                    & QuadGramTable_ex$Midterm1 == lastWords[length-2]
                                                    & QuadGramTable_ex$Midterm2 == lastWords[length-1]
                                                    & QuadGramTable_ex$LastTerm == lastWords[length]]
                if (nrow(oneRecordIn4Gram) > 0){
                        # We found one in 3-gram
                        all_freq = sum(oneGroupIn4Gram$frequency)
                        finalProb = ((oneRecordIn4Gram$discount * oneRecordIn4Gram$frequency) / all_freq)
                        predictionList =oneGroupIn4Gram[which(discount >= finalProb[1]),"LastTerm"]
                        
                        ### We're done!
                } else{
                        # Nothing found in 4 gram look in 3 gram
                        beta_leftoverprob = QuadGramTable_leftOverProb_ex[QuadGramTable_leftOverProb_ex$FirstTerm == lastWords[length-3]
                                                                          & QuadGramTable_leftOverProb_ex$Midterm1 == lastWords[length-2]
                                                                          & QuadGramTable_leftOverProb_ex$Midterm2 == lastWords[length-1]]$leftoverprob
                        ...
                        if (nrow(oneRecordIn3Gram) > 0){
                                ...
                                alpha = beta_leftoverprob / sum((oneGroupIn3Gram_Remain$frequency * oneGroupIn3Gram_Remain$discount) / all_freq)
                                finalProb = alpha * ((oneRecordIn3Gram$frequency * oneRecordIn3Gram$discount ) / all_freq)
                                predictionList =oneGroupIn3Gram[which(discount >= finalProb[1]),"LastTerm"]
                                
                        }else {
                                # NOT found in 3-gram => check 2-gram & 1-gram
                                beta_leftoverprob = TriGramTable_leftOverProb_ex[TriGramTable_leftOverProb_ex$FirstTerm == lastWords[length-3]
                                                                                 & TriGramTable_leftOverProb_ex$Midterm == lastWords[length-2]]$leftoverprob
                                ...
                                if (nrow(oneRecordIn2Gram) > 0){
                                        ...
                                        oneGroupIn2Gram_Remain = oneGroupIn2Gram[!(oneGroupIn2Gram$LastTerm %in% oneGroupI4Gram$LastTerm)]
                                        ...
                                        oneRecordIn1Gram = UniGramTable_ex[UniGramTable_ex$LastTerm == lastWords[length]]
                                        ...
                                        if(nrow(oneRecordIn1Gram) > 0){
                                                oneGroupIn1Gram_Remain = oneGroupIn1Gram[!(oneGroupIn1Gram$LastTerm %in% oneGroupIn4Gram$LastTerm)]
        ...
        } # ends 4 gram condition
        else {
                stop(sprintf("[%s] not found in the 4-gram model.", lastWords[length]))
